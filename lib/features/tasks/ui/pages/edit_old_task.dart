...
class EditOldTaskePage extends StatefulWidget {

final LocalTask  oldTask;
  EditOldTaskePage( {Key? key, required this.oldTask}) : super(key: key);

  @override
  State<EditOldTaskePage> createState() => _EditOldTaskePageState(oldTask);
}

class _EditOldTaskePageState extends State<EditOldTaskePage> {


final LocalTask taskOnEditing;
 // final localTask = LocalTask(title: "",note: "");
  ...
  _EditOldTaskePageState(this.taskOnEditing);



  @override
  Widget build(BuildContext context) {
    _titleController.text=taskOnEditing.title;

    _noteController.text=taskOnEditing.note;
   //  dateC.text = taskOnEditing.creationTime.toString();
     //startC.text = taskOnEditing.startTime.toString();
    //endC.text=taskOnEditing.endTime.toString() ;
    return Directionality(textDirection: ui.TextDirection.rtl, child: Scaffold(
      appBar: build_appBar(title: "تعديل مهمة ", context: context),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            MyInputField(
              title: "التاريخ",
              hint: DateFormat.yMd().format(taskOnEditing.startTime ?? now),
              controller: dateC,
              widget: IconButton(
                icon: Icon(
              ...
              children: [
                Expanded(
                  child: MyInputField(
                    ...
                    hint: DateFormat('hh:mm a')
                        .format(taskOnEditing.startTime ?? now),
                  ),
                ),
                const SizedBox(
                  width: 12,
                ...
                Expanded(
                  child: MyInputField(
                    ...
                    hint:
                    DateFormat('hh:mm a').format(taskOnEditing.endTime ?? now),
                  ),
                ),
              ],
            ),
              ...
              child: SpinnerWidget(
                items: remindList
                    .mapIndexed(
                      ...
                      id: i + 1,
                      item: e,
                      isSelected: e == taskOnEditing.remind,
                      name: 'قبل${remindList[i]} دقائق '),
                )
                    .toList(),
                width: double.infinity,
                onChanged: (p0) {
                  taskOnEditing.remind = p0.item;
                },
              ),
            ),
            const SizedBox(
              ...
              child: SpinnerWidget(
                items: Repeat.values
                    .mapIndexed(
                      ...
                      id: i + 1,
                      item: e,
                      isSelected: e == taskOnEditing.repeat,
                      name: e.getName),
                )
                    .toList(),
                width: double.infinity,
                onChanged: (p0) {
                  taskOnEditing.repeat = p0.item;
                },
              ),
            ),
            Row(
              children: [
                Expanded(
                  child: MyButton(
                    lable: "تعديل",
                    onTap: () {
                      taskOnEditing.title = _titleController.text;
                      taskOnEditing.note = _noteController.text;
  ...
  _getDateFromUser() async {
    ...
    if (pickerDate != null) {
      setState(() {
        taskOnEditing.startTime = taskOnEditing.startTime?.copyWith(
          day: pickerDate.day,
          month: pickerDate.month,
          year: pickerDate.year,
        );

        dateC.text = DateFormat.yMd().format(taskOnEditing.startTime!);
      });
    }
  }
  ...
  _getTimeFromUser({required bool isStartTime}) async {
    ...
    if (isStartTime == true) {
      setState(() {
        ...
        startC.text = DateFormat('hh:mm a').format(taskOnEditing.startTime ?? now);
      });
    } else {
      setState(() {
        taskOnEditing.endTime = taskOnEditing.endTime!.copyWith(
          hour: pickerTime.hour,
          minute: pickerTime.minute,
        );
        endC.text = DateFormat('hh:mm a').format(taskOnEditing.endTime ?? now);
      });
    }
  }
