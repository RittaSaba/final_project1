...
abstract class CaseseDetailsRemoteDataSource {
...
class Casese_detailsRemoteDataSourceImpl
    implements CaseseDetailsRemoteDataSource {
  final http.Client client;

  Casese_detailsRemoteDataSourceImpl(
      this.id, this.title, this.facts, this.legal_discussion, this.id_cases,
  ...
  Future<List<Casese_detailsModel>> getAllCasese_details() async {
    ...
    if (response.statusCode == 200) {
      ...
      final List<Casese_detailsModel> casese_detailsModels = decodedJson
          .map<Casese_detailsModel>((jsonCasese_detailsModel) =>
              Casese_detailsModel.fromJson(jsonCasese_detailsModel))
          .toList();
  ...
  @override
  Future<Unit> updateCasese_details(
      Casese_detailsModel casese_detailsModel) async {
    ...
    final response = await client
        .patch(Uri.parse(BASE_URL + "/posts/$casese_detailsModel"), body: body);
    if (response.statusCode == 200) {
